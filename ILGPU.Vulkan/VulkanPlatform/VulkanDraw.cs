...
namespace VulkanPlatform
{
	public static class VulkanDraw
	{
        public static unsafe void DrawFrame(this IVulkanRenderer renderer)
        {
            ...
            uint imageIndex;
            VkResult acquireResult = VulkanNative.vkAcquireNextImageKHR(renderer.VSupport.Device, renderer.SwapChain, ulong.MaxValue, renderer.ImageAvailableSemaphore, 0, &imageIndex);
            if (acquireResult == VkResult.VK_ERROR_OUT_OF_DATE_KHR || acquireResult == VkResult.VK_SUBOPTIMAL_KHR)
            {
                return;
            }
            VulkanHelpers.CheckErrors(acquireResult);
            ...
            VkResult presentResult = VulkanNative.vkQueuePresentKHR(renderer.PresentQueue, &presentInfo);
            if (presentResult == VkResult.VK_ERROR_OUT_OF_DATE_KHR || presentResult == VkResult.VK_SUBOPTIMAL_KHR)
            {
                return;
            }
            VulkanHelpers.CheckErrors(presentResult);
