...
namespace VulkanPlatform
{
    ...
    public static class VulkanComputing
    {
        ...
        public unsafe static void SubmitAndWait(this IVulkanCompute compute, VkSubmitInfo[] submitInfos)
        {
            ...
            {
                VulkanHelpers.CheckErrors(VulkanNative.vkQueueSubmit(compute.ComputeQueue, (uint)submitInfos.Length, submitInfoPtr, fence));
            }

            VulkanHelpers.CheckErrors(VulkanNative.vkWaitForFences(compute.Support.Device, 1, &fence, true, 100000000));
            VulkanNative.vkDestroyFence(compute.Support.Device, fence, null);
        }
    }
}
